...
//will need to replace file in Debug folder upon load-up
const char meshFile[128] = "../Debug/Mesh/bunny2k.obj";
...
void idle()
{
	...
	/*****************************/
	// theta equals itself plus the deltatime
	// this determines the speed of rotation 
	theta += deltaTime; 

	//camera x and z are determined by the cosine of the angle and the sin of the angle, respectively
	//both values are multiplied by the radius to keep the camera at the correct distance
	cam_x = cos(theta) * radius;
	cam_z = sin(theta) * radius;

	//reshape function is called to properly adjust the camera's position and rotation
