...
{
	//local variable to store the current vertex
	//being checked
	vec3 currentVertex; 
	//loop through the vertices array
	for (int i = 0; i < vertNum; i++) {
		//store in the current vertex in currentVertex variable
		currentVertex = vec3(vertices[i * 3], vertices[i * 3 + 1] , vertices[i * 3 + 2]);
		//check each max/min coord(x, y, and z) against the currentVertex
		// 
		//Case max: if the current vertex has a coord.(x, y, or z) that is greater
		//than the current max. coord, the max coord. is set to the coord of the current
		//vertex
		if (currentVertex.x > maxCoords.x) {
			maxCoords.x = currentVertex.x; 
		}
		if (currentVertex.y > maxCoords.y) {
			maxCoords.y = currentVertex.y;
		}
		if (currentVertex.z > maxCoords.z) {
			maxCoords.z = currentVertex.z;
		}

		//Case min: if the current vertex has a coord.(x, y, or z) that is less
		//than the current min. coord, the min. coord. is set to the coord of the current
		//vertex
		if (currentVertex.x < minCoords.x) {
			minCoords.x = currentVertex.x;
		}
		if (currentVertex.y < minCoords.y) {
			minCoords.y = currentVertex.y;
		}
		if (currentVertex.z < minCoords.z) {
			minCoords.z = currentVertex.z;
		}
	}
}
...
{
	//begin drawing the wireframe cube with GL_LINE_LOOP
	glColor3f(1.0f, 0.1f, 0.1f); 
	glBegin(GL_LINE_LOOP);

	//using line loop, draw the wireframe cube using the max/min coords.
	glVertex3f(minCoords.x, maxCoords.y, maxCoords.z);//min x; max y,z
	glVertex3f(minCoords.x, maxCoords.y, minCoords.z);//min x,z; max y
	glVertex3f(minCoords.x, minCoords.y, minCoords.z);//min x,y,z
	glVertex3f(maxCoords.x, minCoords.y, minCoords.z);//min y,z; max x
	glVertex3f(maxCoords.x, maxCoords.y, minCoords.z);//min z; max x,y
	glVertex3f(maxCoords.x, maxCoords.y, maxCoords.z);//max x,y,z
	glVertex3f(minCoords.x, maxCoords.y, maxCoords.z);//min x; max y,z
	glVertex3f(minCoords.x, minCoords.y, maxCoords.z);//min x,y; max z
	glVertex3f(minCoords.x, minCoords.y, minCoords.z);//min x,y,z
	glVertex3f(minCoords.x, maxCoords.y, minCoords.z);//min x,z; max y
	glVertex3f(maxCoords.x, maxCoords.y, minCoords.z);//min z; max x,y
	glVertex3f(maxCoords.x, minCoords.y, minCoords.z);//min y,z; max x
	glVertex3f(maxCoords.x, minCoords.y, maxCoords.z);//min y; max x,z
	glVertex3f(maxCoords.x, maxCoords.y, maxCoords.z);//max x,y,z
	glVertex3f(minCoords.x, maxCoords.y, maxCoords.z);//min x; max y,z
	glVertex3f(minCoords.x, minCoords.y, maxCoords.z);//min x,y; max z
	glVertex3f(maxCoords.x, minCoords.y, maxCoords.z);//min y; max x,z
